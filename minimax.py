	...
	for i in range(len(node.horizontal_values)):
		if node.horizontal_values[i] == 0:
			...
			if new_node.current_player == 1:
				ret_val = find_maximum(new_node)
			else:
				ret_val = find_minimum(new_node)

			# this part determine the min - max val
			if ret_val[0] > max_val[0]:
				max_val = ret_val
			elif ret_val[0] == max_val[0]:
				if ret_val[1] < max_val[1]:
					max_val = ret_val
...
def find_minimum(node):
	...
	for i in range(len(node.horizontal_values)):
		if node.horizontal_values[i] == 0:
			...
			# this part determine the min - max val
			if ret_val[1] > min_val[1]:
				min_val = ret_val
			elif ret_val[1] == min_val[1]:
				if ret_val[0] < min_val[0]:
					min_val = ret_val
